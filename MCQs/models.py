...
class Test(models.Model):
    ...
    def distribute_scores(self, data):
        ...
        for student_id, answer_list in data.items():
            ...
            points = 0
            new_note = Note(test=self, student=student)

            for i, answer in enumerate(answer_list):
                if answer == correct_answer_list[i]:
                    points += 1
                else:
                    # Add the question to the note
                    print(self.selected_questions.all())
                    print("\n\n")
                    question = self.selected_questions.get(order=i)
                    new_note.questions += f"{question.question} Correct answer: {question.get_correct_ans()}\n"
